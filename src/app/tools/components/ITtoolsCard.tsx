...
export default function ITtoolsCard() {
    const itTools = [
        {
            ...
            labelBackground: "#247CC0",
            // icon: "/it-tools/tracker.svg",
            title: "Renewal tracker",
            url: "/tools/offboard-it",
            description: "Never miss a SaaS renewal. Organize contracts in one view, track upcoming renewal dates, and stay ahead with automated email & calendar reminders.",
        },
        {
            label: "Employee Offboarding",
            labelBackground: "#F6A36D",
            // icon: "/it-tools/offboardIT.svg",
            title: "OffboardIT",
            url: "/tools/renewal-tracker",
            description: "Streamline your org’s offboardingprocess. Create a customized list of tasks based on the departing employee's roleand department.",
        },
        {
            label: "Access Management",
            labelBackground: "#F8BE65",
    ...
    return (
        <section className="flex flex-col items-center gap-6 pt-14 pb-0 md:py-20 relative w-full z-[2] px-6 bg-[#f8f5f3]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-[1256px] ">
                ...
                .map((tool, index) => (
                    ...
                    >
                        <div className={`bg-[${tool.labelBackground}] rounded-4xl px-4 py-1 text-white mb-6`}>{tool.label}</div>
                        <div className={`font-medium text-[${tool.labelBackground}] text-3xl`}>{tool.title}</div>
